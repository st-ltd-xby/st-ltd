...
// api/[[path]].js
var BACKEND = "https://st-ltd-api-production.up.railway.app";
...
// uploads/[[path]].js
var BACKEND2 = "https://st-ltd-api-production.up.railway.app";
async function onRequest2({ request }) {
  const url = new URL(request.url);
  const target = BACKEND2 + url.pathname + url.search;
  const headers = new Headers(request.headers);
  headers.delete("host");
  headers.delete("origin");
  headers.delete("referer");
...
// ../.wrangler/tmp/pages-9nGR6J/functionsRoutes-0.06392990815505062.mjs
var routes = [
  {
    routePath: "/api/:path*",
    mountPath: "/api",
