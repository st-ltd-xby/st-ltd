        ...
        try {
            ...
            if (parsedTags.externalUrl) {
                return {
                    ...
                    tags: JSON.stringify(Object.fromEntries(Object.entries(parsedTags).filter(([key]) => key !== 'externalUrl'))),
                };
            }
        }
        catch {
...
router.get('/products', async (req, res) => {
    try {
        ...
        const [products, total] = await Promise.all([
            prisma_1.default.product.findMany({ where, skip, take: Number(pageSize), include: { skus: true }, orderBy: { sortOrder: 'asc' } }),
            prisma_1.default.product.count({ where }),
        ]);
        // 解析产品数据，提取 externalUrl 信息
        const parsedProducts = products.map(parseProductData);
        (0, response_1.successWithPagination)(res, parsedProducts, { page: Number(page), pageSize: Number(pageSize), total });
    }
    catch (error) {
...
router.get('/orders', async (req, res) => {
    try {
        ...
        const [orders, total] = await Promise.all([
            prisma_1.default.order.findMany({ where, skip, take: Number(pageSize), include: { items: true, payments: true }, orderBy: { createdAt: 'desc' } }),
            prisma_1.default.order.count({ where }),
        ]);
        (0, response_1.successWithPagination)(res, orders, { page: Number(page), pageSize: Number(pageSize), total });
    }
    catch (error) {
        (0, response_1.fail)(res, error.message);
    }
    ...
    try {
        ...
        const order = await prisma_1.default.order.create({
            data: {
                ...
                items: { create: items.map((item) => ({ productId: item.productId, name: item.name, price: item.price, quantity: item.quantity, subtotal: item.price * item.quantity })) },
            },
            include: { items: true },
        });
        (0, response_1.success)(res, order, '订单创建成功');
    }
    catch (error) {
        (0, response_1.fail)(res, error.message);
    }
...
router.put('/orders/:id', async (req, res) => {
    try {
        ...
        (0, response_1.success)(res, order, '订单更新成功');
    }
    catch (error) {
        (0, response_1.fail)(res, error.message);
    }
...
router.get('/appointments', async (req, res) => {
    try {
        ...
        (0, response_1.success)(res, appointments);
    }
    catch (error) {
        (0, response_1.fail)(res, error.message);
    }
...
router.post('/appointments', async (req, res) => {
    try {
        ...
        (0, response_1.success)(res, appointment, '预约创建成功');
    }
    catch (error) {
        (0, response_1.fail)(res, error.message);
    }
...
// ===== 外部网站接入管理 =====
// 获取外部网站列表
router.get('/external-websites', async (req, res) => {
    try {
        // 使用系统配置表存储外部网站信息
        ...
        (0, response_1.success)(res, websites);
    }
    catch (error) {
        (0, response_1.fail)(res, error.message);
    }
...
// 添加外部网站
router.post('/external-websites', async (req, res) => {
    try {
        ...
        try {
            ...
            return (0, response_1.fail)(res, '无效的网站URL');
        }
        // 生成唯一ID
        const websiteId = `extws_${Date.now()}_${Math.random().toString(36).substr(2, 5)}`;
        // 存储到系统配置表
        const website = await prisma_1.default.systemConfig.create({
            data: {
...
// 更新外部网站
router.put('/external-websites/:id', async (req, res) => {
    try {
        const { name, url, platform, apiKey, apiSecret, status } = req.body;
        // 从系统配置表获取现有数据
        const existingConfig = await prisma_1.default.systemConfig.findUnique({
            where: {
                tenantId_key: {
                    tenantId: req.user.tenantId,
        ...
        if (!existingConfig) {
            return (0, response_1.notFound)(res, '外部网站不存在');
        }
        // 解析现有数据并合并更新
        ...
        const updatedConfig = await prisma_1.default.systemConfig.update({
...
// 删除外部网站
router.delete('/external-websites/:id', async (req, res) => {
    try {
        ...
        (0, response_1.success)(res, null, '外部网站删除成功');
    }
    catch (error) {
        (0, response_1.fail)(res, error.message);
    }
...
// 同步外部网站产品
router.post('/external-websites/:id/sync', async (req, res) => {
    try {
        // 获取网站配置
        ...
        for (const product of sampleProducts) {
            ...
            // 检查是否已存在
            const existingProduct = await prisma_1.default.systemConfig.findUnique({
                where: {
                    tenantId_key: {
                        tenantId: req.user.tenantId,
...
router.get('/external-products', async (req, res) => {
    try {
        ...
        (0, response_1.success)(res, products);
    }
    catch (error) {
        (0, response_1.fail)(res, error.message);
    }
...
// 获取推广任务列表
router.get('/promotion-tasks', async (req, res) => {
    try {
        // 从系统配置表获取推广任务
        const configs = await prisma_1.default.systemConfig.findMany({
            where: {
        ...
        (0, response_1.success)(res, tasks);
    }
    catch (error) {
        (0, response_1.fail)(res, error.message);
    }
...
// 创建推广任务
router.post('/promotion-tasks', async (req, res) => {
    try {
        const { name, externalProductId, type, config: taskConfig } = req.body;
        // 生成任务ID
        const taskId = `task_${Date.now()}_${Math.random().toString(36).substr(2, 5)}`;
        // 创建推广任务
        const task = await prisma_1.default.systemConfig.create({
            data: {
                tenantId: req.user.tenantId,
...
router.get('/contents', async (req, res) => {
    try {
        ...
        const skip = (Number(page) - 1) * Number(pageSize);
        const where = { tenantId: req.user.tenantId, key: { startsWith: 'content_' } };
        if (type)
            where.value = { contains: `"type":"${type}"` };
        if (status)
            where.value = { contains: `"status":"${status}"` };
        const [configs, total] = await Promise.all([
            prisma_1.default.systemConfig.findMany({
                where,
                skip,
                take: Number(pageSize),
                orderBy: { createdAt: 'desc' },
            }),
            prisma_1.default.systemConfig.count({ where }),
        ]);
        ...
        (0, response_1.successWithPagination)(res, contents, { page: Number(page), pageSize: Number(pageSize), total });
    }
    catch (error) {
        (0, response_1.fail)(res, error.message);
    }
...
router.post('/contents', async (req, res) => {
    try {
        ...
        if (!validTypes.includes(type)) {
            return (0, response_1.fail)(res, '无效的内容类型，支持: article(图文), video(视频), whitepaper(白皮书)');
        }
        const contentId = `content_${Date.now()}_${Math.random().toString(36).substr(2, 5)}`;
        const content = await prisma_1.default.systemConfig.create({
            data: {
                tenantId: req.user.tenantId,
...
// 发布内容
router.post('/contents/:id/publish', async (req, res) => {
    try {
        const config = await prisma_1.default.systemConfig.findUnique({
            where: {
                tenantId_key: {
...
router.get('/employee-tasks', async (req, res) => {
    try {
        ...
        const skip = (Number(page) - 1) * Number(pageSize);
        const where = { tenantId: req.user.tenantId, key: { startsWith: 'employee_task_' } };
        if (status)
            where.value = { contains: `"status":"${status}"` };
        if (assigneeId)
            where.value = { contains: `"assigneeId":"${assigneeId}"` };
        const [configs, total] = await Promise.all([
            prisma_1.default.systemConfig.findMany({
                where,
                skip,
        ...
        (0, response_1.successWithPagination)(res, tasks, { page: Number(page), pageSize: Number(pageSize), total });
    }
    catch (error) {
        (0, response_1.fail)(res, error.message);
    }
...
// 更新员工任务
router.put('/employee-tasks/:id', async (req, res) => {
    try {
        ...
        if (!existingConfig) {
            return (0, response_1.notFound)(res, '任务不存在');
        }
        const existingData = JSON.parse(existingConfig.value);
        const updatedData = {
            ...existingData,
...
// 删除员工任务
router.delete('/employee-tasks/:id', async (req, res) => {
    try {
        await prisma_1.default.systemConfig.delete({
            where: {
                tenantId_key: {
        ...
        (0, response_1.success)(res, null, '任务删除成功');
    }
    catch (error) {
        (0, response_1.fail)(res, error.message);
    }
...
// 更新任务进度
router.post('/employee-tasks/:id/progress', async (req, res) => {
    try {
...
router.get('/employee-tasks', async (req, res) => {
    try {
        ...
        if (status)
            where.value = { contains: `"status":"${status}"` };
        if (assignee)
            where.value = { contains: `"assignee":"${assignee}"` };
        const [configs, total] = await Promise.all([
            prisma_1.default.systemConfig.findMany({
                where,
                skip,
...
// 更新任务进度
router.put('/employee-tasks/:id/progress', async (req, res) => {
    try {
...
router.get('/employees', async (req, res) => {
    try {
        ...
        (0, response_1.success)(res, employees);
    }
    catch (error) {
        (0, response_1.fail)(res, error.message);
    }
...
router.get('/leads', async (req, res) => {
    try {
        ...
        const skip = (Number(page) - 1) * Number(pageSize);
        const where = { tenantId: req.user.tenantId, key: { startsWith: 'lead_' } };
        if (status)
            where.value = { contains: `"status":"${status}"` };
        if (source)
            where.value = { contains: `"source":"${source}"` };
        if (assignee)
            where.value = { contains: `"assignee":"${assignee}"` };
        const [configs, total] = await Promise.all([
            prisma_1.default.systemConfig.findMany({
...
router.post('/leads', async (req, res) => {
    try {
        ...
        const lead = await prisma_1.default.systemConfig.create({
            data: {
                tenantId: req.user.tenantId,
                key: `lead_${leadId}`,
                value: JSON.stringify({
...
router.get('/customers', async (req, res) => {
    try {
        ...
        const skip = (Number(page) - 1) * Number(pageSize);
        const where = { tenantId: req.user.tenantId, key: { startsWith: 'customer_' } };
        if (status)
            where.value = { contains: `"status":"${status}"` };
        if (industry)
            where.value = { contains: `"industry":"${industry}"` };
        if (assignee)
            where.value = { contains: `"assignee":"${assignee}"` };
        const [configs, total] = await Promise.all([
            prisma_1.default.systemConfig.findMany({
        ...
        (0, response_1.successWithPagination)(res, customers, { page: Number(page), pageSize: Number(pageSize), total });
    }
    catch (error) {
        (0, response_1.fail)(res, error.message);
    }
...
router.post('/customers', async (req, res) => {
    try {
        ...
        const customer = await prisma_1.default.systemConfig.create({
            data: {
                tenantId: req.user.tenantId,
        ...
        if (leadId) {
            const leadConfig = await prisma_1.default.systemConfig.findUnique({
                where: {
                    tenantId_key: {
                        tenantId: req.user.tenantId,
                        key: `lead_${leadId}`
...
router.get('/opportunities', async (req, res) => {
    try {
        ...
        const skip = (Number(page) - 1) * Number(pageSize);
        const where = { tenantId: req.user.tenantId, key: { startsWith: 'opportunity_' } };
        if (status)
            where.value = { contains: `"status":"${status}"` };
        if (assignee)
            where.value = { contains: `"assignee":"${assignee}"` };
        if (leadId)
            where.value = { contains: `"leadId":"${leadId}"` };
        const [configs, total] = await Promise.all([
            prisma_1.default.systemConfig.findMany({
                where,
                skip,
        ...
        (0, response_1.successWithPagination)(res, opportunities, { page: Number(page), pageSize: Number(pageSize), total });
    }
    catch (error) {
        (0, response_1.fail)(res, error.message);
    }
...
router.post('/opportunities', async (req, res) => {
    try {
        ...
        const opportunity = await prisma_1.default.systemConfig.create({
            data: {
                tenantId: req.user.tenantId,
...
// 更新商机进度
router.put('/opportunities/:id/progress', async (req, res) => {
    try {
...
router.get('/sites', async (req, res) => {
    try {
        ...
        if (status)
            whereConditions.value = { contains: status.toString() };
        if (category)
            whereConditions.value = { contains: category.toString() };
        const sites = await prisma_1.default.systemConfig.findMany({
            where: whereConditions,
            skip,
            take: Number(pageSize),
            orderBy: { createdAt: 'desc' },
        });
        const totalCount = await prisma_1.default.systemConfig.count({ where: whereConditions });
...
router.post('/sites', async (req, res) => {
    try {
        const { customerName, customerCompany, customerPhone, customerEmail, siteName, siteUrl, siteCategory, description, notes } = req.body;
        ...
        (0, response_1.success)(res, { id: siteId, ...siteData }, '站点创建成功');
    }
    catch (error) {
        (0, response_1.fail)(res, error.message);
    }
...
router.put('/sites/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const { customerName, customerCompany, customerPhone, customerEmail, siteName, siteUrl, siteCategory, description, notes, status } = req.body;
        const existingConfig = await prisma_1.default.systemConfig.findUnique({
            where: {
                tenantId_key: {
                    tenantId: req.user.tenantId,
        ...
        (0, response_1.success)(res, { id, ...updatedData }, '站点更新成功');
    }
    catch (error) {
        (0, response_1.fail)(res, error.message);
    }
...
router.delete('/sites/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const result = await prisma_1.default.systemConfig.delete({
            where: {
                tenantId_key: {
                    tenantId: req.user.tenantId,
                    key: `site_${id}`
        ...
        (0, response_1.success)(res, { id }, '站点删除成功');
    }
    catch (error) {
        (0, response_1.fail)(res, error.message);
    }
...
router.get('/pages', async (req, res) => {
    try {
        ...
        if (category)
            whereConditions.value = { contains: category.toString() };
        if (templateType)
            whereConditions.value = { contains: templateType.toString() };
        const pages = await prisma_1.default.systemConfig.findMany({
            where: whereConditions,
            skip,
            take: Number(pageSize),
            orderBy: { createdAt: 'desc' },
...
router.post('/pages', async (req, res) => {
    try {
        const { customerName, customerCompany, customerPhone, customerEmail, pageName, pageDescription, templateType, content, status } = req.body;
        ...
        (0, response_1.success)(res, { id: pageId, ...pageData }, '推广页面创建成功');
    }
    catch (error) {
        (0, response_1.fail)(res, error.message);
    }
...
router.put('/pages/:id', async (req, res) => {
    try {
        ...
        if (!existingConfig) {
            return (0, response_1.notFound)(res, '推广页面不存在');
        }
        const existingData = JSON.parse(existingConfig.value);
        const updatedData = {
            ...existingData,
        ...
        (0, response_1.success)(res, { id, ...updatedData }, '推广页面更新成功');
    }
    catch (error) {
        (0, response_1.fail)(res, error.message);
    }
...
router.delete('/pages/:id', async (req, res) => {
    try {
        ...
        (0, response_1.success)(res, { id }, '推广页面删除成功');
    }
    catch (error) {
        (0, response_1.fail)(res, error.message);
    }
...
router.post('/pages/:id/publish', async (req, res) => {
    try {
        ...
        (0, response_1.success)(res, { id, ...existingData }, '推广页面发布成功');
    }
    catch (error) {
        (0, response_1.fail)(res, error.message);
    }
...
router.get('/pages/:id/preview', async (req, res) => {
    try {
        ...
        (0, response_1.success)(res, pageData, '推广页面预览数据获取成功');
    }
    catch (error) {
        (0, response_1.fail)(res, error.message);
    }
...
router.post('/pages/:id/generate-link', async (req, res) => {
    try {
        ...
        if (pageData.status !== 'published') {
            return (0, response_1.fail)(res, '页面必须先发布才能生成外网访问链接');
        }
        // 生成页面访问链接
        let pageUrl;
        if (customDomain) {
        ...
        (0, response_1.success)(res, { pageUrl }, '页面访问链接生成成功');
    }
    catch (error) {
        (0, response_1.fail)(res, error.message);
    }
...
// 获取公共页面数据（供外网访问）
router.get('/public/page/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const config = await prisma_1.default.systemConfig.findUnique({
            where: {
        ...
        if (pageData.status !== 'published' || !pageData.publicAccess) {
            return (0, response_1.fail)(res, '页面尚未发布或不允许公开访问');
        }
        // 返回页面HTML内容
...
router.get('/dashboard/stats', async (req, res) => {
    try {
        ...
        // 获取商品总数
        const totalProducts = await prisma_1.default.product.count({ where: { tenantId } });
        // 获取订单总数
        const totalOrders = await prisma_1.default.order.count({ where: { tenantId } });
        // 获取会员总数
        const totalMembers = await prisma_1.default.member.count({ where: { tenantId } });
        // 获取预约总数
        const totalAppointments = await prisma_1.default.appointment.count({ where: { tenantId } });
        // 获取外部网站总数
        const totalExternalWebsites = await prisma_1.default.systemConfig.count({
            where: { tenantId, key: { startsWith: 'external_website_' } }
        });
        // 获取内容总数
        const totalContents = await prisma_1.default.systemConfig.count({
            where: { tenantId, key: { startsWith: 'content_' } }
        });
        // 获取员工任务总数
        const totalEmployeeTasks = await prisma_1.default.systemConfig.count({
            where: { tenantId, key: { startsWith: 'employee_task_' } }
        });
        // 获取线索总数
        const totalLeads = await prisma_1.default.systemConfig.count({
            where: { tenantId, key: { startsWith: 'lead_' } }
        });
        // 获取客户总数
        const totalCustomers = await prisma_1.default.systemConfig.count({
            where: { tenantId, key: { startsWith: 'customer_' } }
        });
        // 获取商机总数
        const totalOpportunities = await prisma_1.default.systemConfig.count({
            where: { tenantId, key: { startsWith: 'opportunity_' } }
        });
        // 获取站点总数
        const totalSites = await prisma_1.default.systemConfig.count({
            where: { tenantId, key: { startsWith: 'site_' } }
        });
        // 获取页面总数
        const totalPages = await prisma_1.default.systemConfig.count({
            where: { tenantId, key: { startsWith: 'page_' } }
        });
        // 获取表单总数
        const totalForms = await prisma_1.default.systemConfig.count({
            where: { tenantId, key: { startsWith: 'form_' } }
        });
        // 获取本月新增数据
        const startOfMonth = new Date(new Date().getFullYear(), new Date().getMonth(), 1);
        const monthlyNewProducts = await prisma_1.default.product.count({
            where: { tenantId, createdAt: { gte: startOfMonth } }
        });
        const monthlyNewOrders = await prisma_1.default.order.count({
            where: { tenantId, createdAt: { gte: startOfMonth } }
        });
        const monthlyNewMembers = await prisma_1.default.member.count({
            where: { tenantId, createdAt: { gte: startOfMonth } }
        });
        const monthlyNewLeads = await prisma_1.default.systemConfig.count({
            where: { tenantId, key: { startsWith: 'lead_' }, value: { contains: startOfMonth.toISOString() } }
        });
        // 获取订单统计
        const orders = await prisma_1.default.order.findMany({ where: { tenantId } });
...
router.get('/forms', async (req, res) => {
    try {
        ...
        if (category)
            whereConditions.value = { contains: category.toString() };
        const forms = await prisma_1.default.systemConfig.findMany({
            where: whereConditions,
            skip,
            take: Number(pageSize),
            orderBy: { createdAt: 'desc' },
...
router.post('/forms', async (req, res) => {
    try {
        const { customerName, customerCompany, customerPhone, customerEmail, formName, formType, fields, status, description } = req.body;
        ...
        (0, response_1.success)(res, { id: formId, ...formData }, '表单创建成功');
    }
    catch (error) {
        (0, response_1.fail)(res, error.message);
    }
...
router.put('/forms/:id', async (req, res) => {
    try {
        ...
        if (!existingConfig) {
            return (0, response_1.notFound)(res, '表单不存在');
        }
        const existingData = JSON.parse(existingConfig.value);
        const updatedData = {
            ...existingData,
        ...
        (0, response_1.success)(res, { id, ...updatedData }, '表单更新成功');
    }
    catch (error) {
        (0, response_1.fail)(res, error.message);
    }
...
router.delete('/forms/:id', async (req, res) => {
    try {
        ...
        (0, response_1.success)(res, { id }, '表单删除成功');
    }
    catch (error) {
        (0, response_1.fail)(res, error.message);
    }
...
router.get('/form-submissions', async (req, res) => {
    try {
        ...
        if (formId)
            whereConditions.value = { contains: formId.toString() };
        const submissions = await prisma_1.default.systemConfig.findMany({
            where: whereConditions,
            skip,
            take: Number(pageSize),
            orderBy: { createdAt: 'desc' },
...
router.post('/forms/:id/submit', async (req, res) => {
    try {
        ...
        // 验证表单是否存在且处于活动状态
        const formConfig = await prisma_1.default.systemConfig.findUnique({
            where: {
                tenantId_key: {
                    tenantId: req.user.tenantId,
                    key: `form_${id}`
        ...
        (0, response_1.success)(res, { id: submissionId }, '表单提交成功');
    }
    catch (error) {
        (0, response_1.fail)(res, error.message);
    }
...
//# sourceMappingURL=mall.routes.js.map