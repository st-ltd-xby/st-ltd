...
router.get('/tracking-links', async (req, res) => {
    try {
        ...
        const [links, total] = await Promise.all([
            prisma_1.default.trackingLink.findMany({
                where,
            ...
            prisma_1.default.trackingLink.count({ where }),
        ]);
        (0, response_1.successWithPagination)(res, links, { page: Number(page), pageSize: Number(pageSize), total });
    }
    catch (error) {
...
// 模拟点击追踪链接
router.post('/tracking-links/:id/simulate-click', async (req, res) => {
    try {
        const link = await prisma_1.default.trackingLink.findUnique({
            where: { id: req.params.id },
        });
        if (!link)
            return (0, response_1.notFound)(res, '链接不存在');
        const clicks = Number(req.body.clicks) || 1;
        await prisma_1.default.trackingLink.update({
            where: { id: link.id },
            data: { clickCount: { increment: clicks } },
        });
        (0, response_1.success)(res, { clickCount: link.clickCount + clicks }, `模拟点击 ${clicks} 次成功`);
    }
    catch (error) {
        (0, response_1.fail)(res, error.message);
    }
...
router.delete('/tracking-links/:id', async (req, res) => {
    try {
        ...
        (0, response_1.success)(res, null, '追踪链接删除成功');
    }
    catch (error) {
        (0, response_1.fail)(res, error.message);
    }
...
router.post('/tracking-links/:shortCode/click', async (req, res) => {
    try {
        const link = await prisma_1.default.trackingLink.findUnique({ where: { shortCode: req.params.shortCode } });
        if (!link)
            return (0, response_1.notFound)(res, '链接不存在');
        await prisma_1.default.trackingLink.update({
            where: { id: link.id },
        ...
        (0, response_1.success)(res, { targetUrl: link.targetUrl });
    }
    catch (error) {
        (0, response_1.fail)(res, error.message);
    }
...
// 追踪链接统计
router.get('/tracking-links/stats', async (req, res) => {
    try {
...
router.get('/qrcodes', async (req, res) => {
    try {
        const qrcodes = await prisma_1.default.qrCode.findMany({
            where: { tenantId: req.user.tenantId },
            orderBy: { createdAt: 'desc' },
        });
        (0, response_1.success)(res, qrcodes);
    }
    catch (error) {
        (0, response_1.fail)(res, error.message);
    }
...
router.post('/qrcodes', async (req, res) => {
    try {
        ...
        const qrcode = await prisma_1.default.qrCode.create({
            data: {
        ...
        (0, response_1.success)(res, qrcode, '二维码生成成功');
    }
    catch (error) {
        (0, response_1.fail)(res, error.message);
    }
...
router.put('/qrcodes/:id', async (req, res) => {
    try {
        ...
        const qrcode = await prisma_1.default.qrCode.update({
            where: { id: req.params.id },
            data: { name, targetUrl: normalizedUrl, size, color, bgColor, ...(imageUrl && { imageUrl }) },
        });
        (0, response_1.success)(res, qrcode, '二维码更新成功');
    }
    catch (error) {
        (0, response_1.fail)(res, error.message);
    }
...
router.delete('/qrcodes/:id', async (req, res) => {
    try {
        await prisma_1.default.qrCode.delete({ where: { id: req.params.id } });
        (0, response_1.success)(res, null, '二维码删除成功');
    }
    catch (error) {
        (0, response_1.fail)(res, error.message);
    }
...
// 直接生成二维码图片（不保存）
router.post('/qrcodes/generate', async (req, res) => {
    try {
        ...
        (0, response_1.success)(res, { imageUrl: qrDataUrl });
    }
    catch (error) {
        (0, response_1.fail)(res, error.message);
    }
...
router.post('/email-templates', async (req, res) => {
    try {
        ...
        (0, response_1.success)(res, template, '模板创建成功');
    }
    catch (error) {
        (0, response_1.fail)(res, error.message);
    }
...
router.put('/email-templates/:id', async (req, res) => {
    try {
        ...
        (0, response_1.success)(res, template, '模板更新成功');
    }
    catch (error) {
        (0, response_1.fail)(res, error.message);
    }
...
router.delete('/email-templates/:id', async (req, res) => {
    try {
        await prisma_1.default.emailTemplate.delete({ where: { id: req.params.id } });
        (0, response_1.success)(res, null, '模板删除成功');
    }
    catch (error) {
        (0, response_1.fail)(res, error.message);
    }
...
router.get('/email-campaigns', async (req, res) => {
    try {
        ...
        (0, response_1.success)(res, campaigns);
    }
    catch (error) {
        (0, response_1.fail)(res, error.message);
    }
...
router.post('/email-campaigns', async (req, res) => {
    try {
        ...
        if (scheduledAt) {
            ...
            if (delay > 0) {
                setTimeout(async () => {
                    try {
                        // 重新获取最新的活动信息
                        const latestCampaign = await prisma_1.default.emailCampaign.findUnique({ where: { id: campaign.id } });
                        if (latestCampaign && latestCampaign.status === 'draft') { // 确保活动尚未发送
                            ...
                            // 获取 SMTP 配置
                            const smtpConfig = await prisma_1.default.systemConfig.findFirst({
        ...
        (0, response_1.success)(res, campaign, '邮件活动创建成功');
    }
    catch (error) {
        (0, response_1.fail)(res, error.message);
    }
...
router.post('/email-campaigns/:id/send', async (req, res) => {
    try {
        ...
        if (!campaign)
            return (0, response_1.notFound)(res, '邮件活动不存在');
        // 更新状态为发送中
        await prisma_1.default.emailCampaign.update({
        ...
        if (smtpConfig) {
            try {
                ...
                return (0, response_1.fail)(res, `SMTP 配置错误: ${error.message}`);
            }
        }
        else {
            // 没有 SMTP 配置，模拟发送
        ...
        (0, response_1.success)(res, { successCount, failCount, total: successCount + failCount }, '邮件发送完成');
    }
    catch (error) {
        (0, response_1.fail)(res, error.message);
    }
...
router.delete('/email-campaigns/:id', async (req, res) => {
    try {
        await prisma_1.default.emailCampaign.delete({ where: { id: req.params.id } });
        (0, response_1.success)(res, null, '活动删除成功');
    }
    catch (error) {
        (0, response_1.fail)(res, error.message);
    }
...
router.get('/email-addresses', async (req, res) => {
    try {
        ...
        (0, response_1.success)(res, addresses, '邮件地址列表获取成功');
    }
    catch (error) {
        (0, response_1.fail)(res, error.message);
    }
...
router.post('/email-addresses', async (req, res) => {
    try {
        ...
        if (!emailRegex.test(email)) {
            return (0, response_1.fail)(res, '邮箱格式不正确');
        }
        // 检查是否已经存在于收件人列表中
        const existingCampaigns = await prisma_1.default.emailCampaign.findMany({
            where: { tenantId: req.user.tenantId },
            select: { recipients: true }
        });
        const allRecipients = new Set();
        ...
        if (allRecipients.has(email)) {
            return (0, response_1.fail)(res, '该邮箱地址已存在');
        }
        // 在响应中返回成功消息，但不实际存储
        (0, response_1.success)(res, { email, name, group }, '邮件地址验证成功');
    }
    catch (error) {
        (0, response_1.fail)(res, error.message);
    }
...
// 批量导入邮件地址
router.post('/email-addresses/import', async (req, res) => {
    try {
        const { csvData, group } = req.body;
        if (!csvData) {
            return (0, response_1.fail)(res, '请提供CSV数据');
        }
        // 解析CSV数据 (假设格式为: 邮箱,姓名 或 邮箱)
        const lines = csvData.split('\n').filter((line) => line.trim() !== '');
        if (lines.length === 0) {
            return (0, response_1.fail)(res, 'CSV数据为空');
        }
        let importedCount = 0;
        let errorCount = 0;
        const errors = [];
...
router.post('/smtp-config', async (req, res) => {
    try {
        ...
        (0, response_1.success)(res, null, 'SMTP 配置保存成功');
    }
    catch (error) {
        (0, response_1.fail)(res, error.message);
    }
...
router.get('/smtp-config', async (req, res) => {
    try {
        ...
        (0, response_1.success)(res, config ? JSON.parse(config.value) : null);
    }
    catch (error) {
        (0, response_1.fail)(res, error.message);
    }
...
router.get('/seo/sitemap', async (req, res) => {
    try {
        ...
        (0, response_1.success)(res, { sites, articles });
    }
    catch (error) {
        (0, response_1.fail)(res, error.message);
    }
...
router.get('/seo/analysis', async (req, res) => {
    try {
        ...
        (0, response_1.success)(res, analysis);
    }
    catch (error) {
        (0, response_1.fail)(res, error.message);
    }
...
// 生成结构化数据 (JSON-LD)
router.get('/seo/structured-data', async (req, res) => {
    try {
        const { siteId } = req.query;
        const where = { tenantId: req.user.tenantId };
        if (siteId)
            where.id = siteId;
        const sites = await prisma_1.default.site.findMany({ where });
        const articles = await prisma_1.default.article.findMany({
            where: { tenantId: req.user.tenantId, status: 'published' },
        });
        ...
        (0, response_1.success)(res, structuredData);
    }
    catch (error) {
        (0, response_1.fail)(res, error.message);
    }
...
// 一键修复 SEO 问题
router.post('/seo/auto-fix', async (req, res) => {
    try {
        ...
        if (!site.seoTitle || site.seoTitle.length < 10) {
            ...
            await prisma_1.default.site.update({ where: { id: siteId }, data: { seoTitle: title } });
            fixed++;
            details.push('SEO标题');
        }
        // 2. 修复站点 SEO 描述
        if (!site.seoDesc || site.seoDesc.length < 50) {
            ...
            await prisma_1.default.site.update({ where: { id: siteId }, data: { seoDesc: desc } });
            fixed++;
            details.push('SEO描述');
        }
        // 3. 修复关键词
        if (!site.seoKeywords) {
            await prisma_1.default.site.update({ where: { id: siteId }, data: { seoKeywords: `${site.name},专业服务,解决方案` } });
            fixed++;
            details.push('关键词');
        }
        // 4. 修复页面 SEO（所有页面）
        for (const page of site.pages) {
            ...
            if (pageFixed) {
                await prisma_1.default.page.update({ where: { id: page.id }, data: updateData });
                fixed++;
            }
        }
...
router.get('/seo/strategies', async (req, res) => {
    try {
        ...
        (0, response_1.success)(res, strategies);
    }
    catch (error) {
        (0, response_1.fail)(res, error.message);
    }
...
router.post('/seo/strategies', async (req, res) => {
    try {
        ...
        if (!name || !target || !rules)
            return (0, response_1.fail)(res, '名称、目标和规则不能为空');
        const strategy = await prisma_1.default.seoStrategy.create({
            data: {
                tenantId: req.user.tenantId,
                name,
        ...
        (0, response_1.success)(res, strategy, '策略创建成功');
    }
    catch (error) {
        (0, response_1.fail)(res, error.message);
    }
...
// 更新策略
router.put('/seo/strategies/:id', async (req, res) => {
    try {
        const id = Array.isArray(req.params.id) ? req.params.id[0] : req.params.id;
        const { name, description, target, rules, isActive, sortOrder } = req.body;
        const existing = await prisma_1.default.seoStrategy.findFirst({
            where: { id, tenantId: req.user.tenantId },
        });
        if (!existing)
            return (0, response_1.notFound)(res, '策略不存在');
        const data = {};
        if (name !== undefined)
            data.name = name;
        ...
        const strategy = await prisma_1.default.seoStrategy.update({ where: { id }, data });
        (0, response_1.success)(res, strategy, '策略更新成功');
    }
    catch (error) {
        (0, response_1.fail)(res, error.message);
    }
...
// 删除策略
router.delete('/seo/strategies/:id', async (req, res) => {
    try {
        const id = Array.isArray(req.params.id) ? req.params.id[0] : req.params.id;
        const existing = await prisma_1.default.seoStrategy.findFirst({
            where: { id, tenantId: req.user.tenantId },
        ...
        if (!existing)
            return (0, response_1.notFound)(res, '策略不存在');
        await prisma_1.default.seoStrategy.delete({ where: { id } });
        (0, response_1.success)(res, null, '策略已删除');
    }
    catch (error) {
        (0, response_1.fail)(res, error.message);
    }
...
// 按策略执行 SEO 修复
router.post('/seo/strategy-fix', async (req, res) => {
    try {
        ...
        if (siteId) {
            ...
            for (const strategy of siteStrategies) {
                ...
                for (const rule of rules) {
                    ...
                    let shouldFix = false;
                    if (condition === 'empty' && !currentValue)
                        shouldFix = true;
                    else if (condition === 'short' && currentValue && currentValue.length < (rule.minLength || 10))
                        shouldFix = true;
                    else if (condition === 'missing' && !currentValue)
                        shouldFix = true;
                    if (shouldFix) {
                        let value = template
                            ? template.replace(/\{siteName\}/g, site.name)
                                ...
                                .replace(/\{pageTitle\}/g, '')
                            : currentValue;
                        await prisma_1.default.site.update({ where: { id: siteId }, data: { [field]: value } });
                        fixed++;
                    ...
                    for (const rule of rules) {
                        ...
                        if (shouldFix) {
                            let value = template
                                ...
                                : currentValue;
                            await prisma_1.default.page.update({ where: { id: page.id }, data: { [field]: value } });
                            fixed++;
                    ...
                    if (shouldFix) {
                        let value = template
                            ? template.replace(/\{siteName\}/g, page.site?.name || '')
                                ...
                                .replace(/\{pageTitle\}/g, page.title)
                            : currentValue;
                        await prisma_1.default.page.update({ where: { id: pageId }, data: { [field]: value } });
                        fixed++;
                        details.push(`${strategy.name}: ${field}`);
                    }
                }
