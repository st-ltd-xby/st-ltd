 ...
 */
router.post('/business-card', async (req: Request, res: Response) => {
  try {
    const { image } = req.body;
    if (!image) return fail(res, '请提供图片');
    ...
    if (ocrData.IsErroredOnProcessing) {
      return fail(res, ocrData.ErrorMessage?.[0] || 'OCR识别失败');
    }
    ...
    if (!fullText.trim()) {
      return fail(res, '未能识别出文字，请拍清晰一些');
    }
    ...
    success(res, info, '识别成功');
  } catch (error: any) {
    console.error('OCR识别错误:', error);
    fail(res, error.message || 'OCR识别失败');
  }
});

// 名片文本解析规则
function parseCardText(text: string) {
  const info: any = { name: '', company: '', phone: '', email: '' };
  ...
  for (const line of lines) {
    if (!info.name) {
      ...
      if (/^[\u4e00-\u9fa5]{2,4}$/.test(line)) {
        if (companySuffixWords.test(line) || companyKeywords.test(line)) continue;
        if (positionKeywords.test(line)) continue;
        info.name = line; continue;
      }
      const cnEn = line.match(/^([\u4e00-\u9fa5]{2,4})\s+[A-Za-z]/);
      if (cnEn) { info.name = cnEn[1]; continue; }
    }
  }
