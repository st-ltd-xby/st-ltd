...
router.post('/form-submit', async (req: Request, res: Response) => {
  try {
    ...
    if (!tenantId) {
      return fail(res, '缺少 tenantId');
    }

    // 查找表单配置
    const formConfig = await prisma.systemConfig.findFirst({
      where: {
        tenantId,
        key: `form_${formId}`,
      },
    ...
    success(res, { id: submissionId }, '提交成功，我们会尽快联系您');
  } catch (error: any) {
...
router.post('/customer-register', async (req: Request, res: Response) => {
  try {
    ...
    if (!name && !contactName) {
      return fail(res, '公司名称或联系人至少填写一项');
    }
    ...
    success(res, { id: customer.id }, '注册成功，我们将尽快与您联系');
  } catch (error: any) {
 ...
 */
router.get('/opportunities/:token', async (req: Request, res: Response) => {
  try {
    ...
    if (!opportunity) {
      return notFound(res, '商机不存在或链接已失效');
    }
