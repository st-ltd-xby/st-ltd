          ...
          for (const rule of rules) {
            ...
            if (field === 'title') {
              if (condition === 'minLength' && site.seoTitle) {
                ...
                addCheck('策略', passed, 
                  `SEO标题过短（${site.seoTitle.length}字符，要求≥${minLen}）`,
                  action || `标题至少 ${minLen} 个字符`,
                  'medium', site.seoTitle);
              }
              if (condition === 'maxLength' && site.seoTitle) {
                const maxLen = parseInt(condition.split(':')[1] || '60');
                const passed = site.seoTitle.length <= maxLen;
                addCheck('策略', passed,
                  `SEO标题过长（${site.seoTitle.length}字符，要求≤${maxLen}）`,
                  action || `标题不超过 ${maxLen} 个字符`,
                  'medium', site.seoTitle);
              }
            ...
            if (field === 'description') {
              if (condition === 'minLength' && site.seoDesc) {
                ...
                addCheck('策略', passed,
                  `SEO描述过短（${site.seoDesc.length}字符，要求≥${minLen}）`,
                  action || `描述至少 ${minLen} 个字符`,
                  'medium', site.seoDesc);
              }
              if (condition === 'maxLength' && site.seoDesc) {
                ...
                addCheck('策略', passed,
                  `SEO描述过长（${site.seoDesc.length}字符，要求≤${maxLen}）`,
                  action || `描述不超过 ${maxLen} 个字符`,
                  'medium', site.seoDesc);
              }
