...
if (fs.existsSync(publicIndexPath)) {
  ...
  console.log('✓ Public index.html loaded (size:', publicIndexHtml.length, 'bytes)');
} else {
  console.warn('⚠ Public index.html not found at', publicIndexPath);
}
...
app.get('/scan', (req, res) => {
  ...
  let html = publicIndexHtml.replace(/<title>[^<]*<\/title>/, '<title>名片扫描 - ST-LTD</title>');
...
app.get('/mobile-visits', (req, res) => {
  ...
  let html = publicIndexHtml.replace(/<title>[^<]*<\/title>/, '<title>移动拜访 - ST-LTD</title>');
  res.setHeader('Content-Type', 'text/html; charset=utf-8');
  res.send(html);
});
