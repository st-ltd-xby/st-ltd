...
export default function Leads() {
  ...
  useEffect(() => { load(); }, []);
  const load = async () => { setLoading(true); try { const res: any = await scrmApi.getLeads(); if (res.code === 0) setLeads(res.data || []); } catch {} finally { setLoading(false); } };
  const handleCreate = async (values: any) => { try { const res: any = await scrmApi.createLead(values); if (res.code === 0) { message.success('线索创建成功'); setModalOpen(false); load(); } } catch {} };
  ...
  const columns = [
    ...
    { title: '来源', dataIndex: 'source', render: (v: string) => <Tag color={sourceColors[v] || 'default'}>{v}</Tag> },
